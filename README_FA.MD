...
> [!NOTE]
> DNS tunneling با محدودیت اندازه payload، رفتار resolverها، latency، rate limit و packet loss روبه‌رو است. هدف CottenDns اتصال قابل استفاده در شرایط سخت است، نه ادعای benchmark غیرواقعی یا جایگزینی VPN معمولی روی شبکه تمیز و پرسرعت.

## 🆕 تازه‌ها

کارهای اخیر روی کارکرد پایدار روی شبکه‌های بسیار محدودکننده و پر-loss متمرکز بوده است:

- **fallback به DNS-over-TCP/53.** سرور هم‌زمان روی UDP/53 و TCP/53 (همان پورت) سرویس می‌دهد و کلاینت (`RESOLVER_TRANSPORT = auto`) ابتدا روی UDP پروب می‌کند و اگر هیچ resolverای پیدا نشد، کل لیست را روی **TCP/53** دوباره پروب می‌کند — برای شبکه‌هایی که UDP/53 را فیلتر یا truncate می‌کنند. وقتی UDP کار می‌کند هیچ هزینه‌ای ندارد. همهٔ کانال‌های پاسخ (TXT/CNAME/A/NULL/HTTPS) روی TCP هم کار می‌کنند.
- **محدودسازی نرخ هوشمند.** resolverای که سیگنال overload می‌دهد (REFUSED/SERVFAIL یا timeout) به‌صورت AIMD کوتاه‌مدت خنک می‌شود و بارش به resolverهای دارای ظرفیت منتقل می‌شود — توزیع مجدد، نه throttle سراسری. خودگِیت است (به resolverهای سالم کاری ندارد)، هرگز بی‌کار نمی‌ماند و از بلاک شدن IP/نرخ جلوگیری می‌کند.
- **بازشکل‌دهی QNAME.** `QNAME_LABEL_LENGTH` داده را در labelهای کوتاه‌تر و jitter-شده می‌چیند به‌جای یک زنجیرهٔ یکنواخت ۶۳ کاراکتری — یک اهرم کاهش fingerprint. برای سرور شفاف است (سرور labelها را بدون توجه به نحوهٔ تقسیم بازسازی می‌کند) پس desync نمی‌شود؛ پیش‌فرض ظرفیت بیشینه را حفظ می‌کند.
- **MTU تطبیقی گروهی.** به‌جای تحمیل MTU کندترین resolver به همه، کلاینت هر resolver را اندازه می‌گیرد و session را روی نقطهٔ بهینهٔ throughput اجرا می‌کند (بیشینهٔ `MTU × تعداد resolverهایی که آن را تحمل می‌کنند`، به‌صورت مشترک روی upload و download). resolverهای کندتر حذف نمی‌شوند بلکه به‌عنوان **ذخیره (reserve)** نگه داشته می‌شوند و در صورت افت pool فعال به‌طور خودکار (با hysteresis برای جلوگیری از نوسان) ارتقا می‌یابند.
- **سه وضعیت مشخص برای resolver.** هر resolver به‌صورت **فعال (active)**، **ذخیره (reserve)** یا **نامعتبر (invalid)** علامت‌گذاری می‌شود و در لاگ‌ها خلاصهٔ `[RESOLVER STATES]` نمایش داده می‌شود.
- **FEC مبتنی بر loss.** کدگذاری Reed-Solomon در مسیر download می‌تواند به‌محض عبور loss از یک آستانه، به‌صورت خودکار و per-stream روشن شود و parity را متناسب با loss افزایش دهد (بدون سربار وقتی مسیر سالم است) — برای کارکرد در شرایط loss بسیار بالا.
- **کانال‌های انتقال بیشتر، پذیرفته‌شده به‌صورت پیش‌فرض.** پاسخ‌های تونل علاوه بر TXT/CNAME/A می‌توانند روی رکوردهای **NULL** و **HTTPS/SVCB** سوار شوند؛ سرور هر نوع query که کلاینت بچرخاند را به‌صورت خودکار می‌پذیرد.
- **توزیع وزن‌دار بر اساس MTU.** یک استراتژی جدید به هر resolver به‌نسبت download MTU آن ترافیک می‌دهد.
- **کش امن‌تر و فضای session بزرگ‌تر.** شروع سریع مبتنی بر لاگ اکنون hybrid است (resolverهای کش‌شده مورد اعتمادند اما لیست جدید/تغییریافته همیشه دوباره اسکن می‌شود) و فضای session ID به ۶۵۵۳۵ افزایش یافت.
...
| `internal/vpnproto` | ساخت، parse، payload و control packing packetهای CottenDns |
...
| 🧰 Service | نصب و start کردن service با نام `cottendns` |
...
| فایل | کاربرد |
| --- | --- |
| `CottenDns_Client_*` | executable کلاینت |
...
CottenDns از فایل‌های TOML استفاده می‌کند. مسیرهای config از environment variable خوانده نمی‌شوند. مسیرهای runtime نسبت به executable/config location و با کمک `internal/runtimepath` و config helperها resolve می‌شوند.
...
کیفیت resolver تعیین می‌کند CottenDns واقعاً قابل استفاده هست یا نه. ممکن است یک resolver برای DNS معمولی خوب باشد، اما برای label بزرگ تونل، queryهای تکراری، name طولانی یا rate limit خاص همان resolver مناسب نباشد. همیشه اجازه دهید کلاینت مسیر واقعی resolver/domain را تست کند.
...
| 🔗 Chaining | یک proxy/panel محلی دیگر را به SOCKS5 listener CottenDns chain کنید |
...
CottenDns بدون هیچ ضمانتی و به‌صورت as-is ارائه می‌شود. مسئولیت نحوه deploy، شبکه‌ای که روی آن استفاده می‌کنید و قانونی بودن استفاده در محل زندگی شما با خود شماست.
...
| 🧱 DNS server دیگری روی همان port اجرا نکنید | سرور CottenDns به UDP/53 برای delegated tunnel traffic نیاز دارد |
...
CottenDns یک fork/derivative مستقل در خانواده MasterDnsVPN است. این repository
توسط `tajirax` نگهداری می‌شود و credit مربوط به کار upstream یعنی Null Route و
پروژه اصلی Master DNS / MasterDnsVPN را حفظ می‌کند.

lineage پروژه:

| مورد | مقدار |
| --- | --- |
| پروژه اصلی | Master DNS / MasterDnsVPN |
| نویسنده اصلی | Amin Mahmoudi |
| fork/source بالادستی | Null Route / NullRoute1970 |
| fork فعلی | CottenDns (مخزن `CottenDns`) |
| نگهدارنده فعلی | tajirax |
| License | MIT License |
